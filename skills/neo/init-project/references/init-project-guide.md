...
`{{SERVICE_NAME}}`, `<context>`, …) are **not** sentinels: they are left intact for `/implement` to fill
per-domain.
...
`/health` standalone. A failed HTTP **bind** still panics (a genuine fatal error). When `/implement` adds a
