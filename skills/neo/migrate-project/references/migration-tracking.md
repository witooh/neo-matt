...
- **One markdown file per target service.** Markdown **by design**: the orchestrator reads it to
  decide resume-vs-fresh and to scope continued work, in the spirit of how `/implement` works a ticket from the configured tracker (or `.scratch/<feature>/issues/`).
