...
description: >
  ...
  `/implement` (or Kiro) to add the first domain. Asks the service identity (Go
  ...
  NOTE: only CREATES the empty skeleton: adding domains / AC / endpoints / tests is the `/implement`
  skill.
---
...
builds and serves `GET /health` immediately and is ready for `/implement` (or Kiro) to add the first domain
with no setup.

> This skill **only creates the empty skeleton**. Authoring domains / AC / endpoints / tests is the
> **`/implement`** skill: point the user there once the project exists.
...
- `internal/core/{domain,usecase}`, gateways, and HTTP handlers are **created through /implement**, not the
...
- Generic steering placeholders (`{{MODULE_PATH}}`, `<context>`, …) are intentionally left unresolved
  for `/implement` to fill per-domain: only the four sentinels are substituted at generation time.
