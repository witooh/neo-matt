...
What the specs must reflect: read it yourself, never from a summary. Look for acceptance criteria in the same order `code-review` looks for a spec:

- **Acceptance criteria, in this order:**
  1. A path the user passed as an argument.
  2. Issue references in the commit messages (`#123`, `Closes #45`, GitLab `!67`, etc.), or the ticket `/implement` is working, fetched via `docs/agents/issue-tracker.md`. Local tracker: `.scratch/<feature>/issues/<NN>-<slug>.md`.
