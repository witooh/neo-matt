...
description: >
  ...
  for a raw glab operation. Reviewing an MR for standards/spec is `code-review`.
  Fixing a failing pipeline is `diagnosing-bugs`, then the fix. Do not spawn
  review or fix agents in this skill.
metadata:
  version: "1.2"
---
...
It does not spawn review or fix agents. For a standards/spec review of a diff, call
the Skill tool with `code-review`. For a red pipeline that needs a diagnosis, call
the Skill tool with `diagnosing-bugs`.
...
**Not this skill:** reviewing an MR ("review MR", "ตรวจ MR") for standards/spec is `code-review`. Fixing review findings or CI failures is `diagnosing-bugs` then the product fix. Do not spawn review/fix agents here.
...
Fetch pipeline status and failed-job logs for an MR or branch. This skill only **inspects** CI. Fixing pipeline failures is `diagnosing-bugs`, then the product fix.
