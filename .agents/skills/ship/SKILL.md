...
- **Three files must share the same version:** `package.json`,
  `.claude-plugin/plugin.json` (Claude Code), and `.grok-plugin/plugin.json`
  (Grok Build). Bump all three in the same step. Marketplace indexes
  (`.claude-plugin/marketplace.json`, `.omp-plugin/marketplace.json`,
  `.grok-plugin/marketplace.json`) have no version field: never touch them for
  versioning. Do not recreate a root `plugin.json` (that re-enters Agent Plugins
  and drops user-invoked skills). The Claude and Grok `skills` arrays must stay
  identical (Grok's manifest takes precedence).
...
`.claude-plugin/plugin.json` and `.grok-plugin/plugin.json` match it. If they
diverge, stop and fix; do not pick one silently.
...
1. Bump the `version` field in `package.json`, `.claude-plugin/plugin.json`,
   and `.grok-plugin/plugin.json` to `<next>`.
